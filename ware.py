...
TOKEN_PATTERNS = [
    ...
    ("KW",       r"\b(while|during|if|else|end|function|which|has|read|show|add|to|break|continue|range|and|or|not|is|bigger|smaller|than|return)\b"),
...
class Parser:
    ...
    def parse_block(self):
        stmts = []
        while self.peek()[1] not in ("end", "else") and self.peek()[0] != "EOF":
            stmts.append(self.parse_stmt())
        return stmts
    ...
    def parse_stmt(self):
        ...
        # assignment: IDENT is <expr>  (but not IDENT is bigger/smaller/not)
